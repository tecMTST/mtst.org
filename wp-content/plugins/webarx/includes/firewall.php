...
class W_Firewall extends W_Core
{
    ...
    public function is_auto_ip_blocked()
    {
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare("SELECT COUNT(*) as numIps FROM " . $wpdb->prefix . "webarx_firewall_log WHERE block_type = 'BLOCK' AND apply_ban = 1 AND ip = '%s' AND log_date >= (STR_TO_DATE(NOW(), '%%Y-%%m-%%d %%H:%%i:%%s') - INTERVAL %d MINUTE)", array($ip, ($this->get_option('webarx_autoblock_minutes', 30) + $this->get_option('webarx_autoblock_blocktime', 60)))), OBJECT
        );

...
class W_Listener extends W_Core
{
    ...
    private function upgradeWebARX()
    {
        ...
        if(!$upgradeResult){
            $this->returnResults(false, null, 'WebARX could not be upgraded, most likely because of invalid filesystem connection information.');
        }

        // Synchronize again with the API.
        do_action('webarx_send_software_data');
        $this->returnResults(null, 'WebARX has been upgraded.');    
    }

...
if ( ! class_exists( 'CBXPetitionEmailEmogrifier' ) ):
	class CBXPetitionEmailEmogrifier {
		/**
		 * @var string
		 */
		...
		public function emogrify() {
			...
			if ( $nodesWithStyleAttributes !== false ) {
				...
				foreach ( $nodesWithStyleAttributes as $node ) {
					$normalizedOriginalStyle = preg_replace_callback( '/[A-z\\-]+(?=\\:)/S',
						array(
							$this,
							'strtolower',
						),
		...
		private function translateCssToXpath( $paramCssSelector ) {
			...
			if ( ! isset( $this->caches[ self::CACHE_KEY_XPATH ][ $xpathKey ] ) ) {
				...
				$cssSelector = preg_replace_callback( self::ID_ATTRIBUTE_MATCHER,
					array(
						$this,
						'matchIdAttributes',
					),
					$cssSelector );
				$cssSelector = preg_replace_callback( self::CLASS_ATTRIBUTE_MATCHER,
					array(
						$this,
